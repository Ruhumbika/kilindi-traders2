...
<header class="border-b border-border bg-card px-6 py-4">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-card-foreground">Add New Debt</h1>
            <p class="text-muted-foreground">Record a new debt for a trader</p>
        </div>
                ...
                <form action="{{ route('debts.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea id="description" name="description" rows="3"
                                  ...
                                  class="w-full border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
